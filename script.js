...
function buildTable() {
  ...
  myLibrary.forEach((book, index) => {
    // uses the value and the index of the current element
    let newRow = document.createElement("tr");
    Object.keys(book).forEach((prop) => {
      // returns an array of property names and initiates callback function for each property
      let newData = document.createElement("td");
      newData.textContent = book[prop]; // text equals the value of the property
      if (prop == "read") {
        // if the property is "read", then do this
